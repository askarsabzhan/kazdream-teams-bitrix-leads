...
describe("UI locale", () => {
  ...
  it("provides the new workflow and usability labels in both locales", () => {
    ...
    expect(getDictionary("ru").detail.notSpecified).toBe("Не указан");
    expect(getDictionary("ru").values.high).toBe("Высокий");
    expect(getDictionary("ru").values.medium).toBe("Средний");
    expect(getDictionary("ru").values.low).toBe("Низкий");
    ...
    expect(getDictionary("en").detail.notSpecified).toBe("Not specified");
    expect(getDictionary("en").values.high).toBe("High");
    expect(getDictionary("en").values.medium).toBe("Medium");
    expect(getDictionary("en").values.low).toBe("Low");
