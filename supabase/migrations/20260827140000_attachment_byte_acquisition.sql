alter table public.attachments
  drop constraint attachments_fetch_state_check;

alter table public.attachments
  add column fetch_attempts integer not null default 0,
  add column fetch_lease_id uuid,
  add column fetch_lease_expires_at timestamptz,
  add column last_fetch_attempt_at timestamptz,
  add column last_error_code text,
  add column acquired_at timestamptz,
  add constraint attachments_fetch_state_check
    check (
      fetch_state in (
        'pending',
        'downloading',
  ...
  add constraint attachments_fetch_attempts_check
    check (fetch_attempts >= 0),
  add constraint attachments_fetch_lease_check
    check (
      (
        fetch_state = 'downloading'
        and fetch_lease_id is not null
        and fetch_lease_expires_at is not null
      )
      or (
        fetch_state <> 'downloading'
  ...
  add constraint attachments_sha256_check
    check (sha256 is null or sha256 ~ '^[0-9a-f]{64}$'),
  add constraint attachments_last_error_code_check
    check (
      last_error_code is null
      or last_error_code ~ '^[A-Z0-9_]{1,64}$'
    ),
  add constraint attachments_fetched_metadata_check
    check (
      fetch_state <> 'fetched'
      or (
...
create index attachments_acquisition_claim_idx
...
comment on column public.attachments.fetch_state is
  'Durable byte-acquisition state. downloading is lease-bound and stale leases are reclaimable.';
comment on column public.attachments.mime_type is
  'Magic-byte-validated content type after acquisition; source declarations remain in source_content_type and source_locator.';
comment on column public.attachments.size_bytes is
  'Exact downloaded byte length after successful acquisition.';
comment on column public.attachments.storage_path is
  'Private deterministic Storage object path; never a public or signed URL.';

create function public.preserve_fetched_attachment_acquisition_metadata()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  if old.fetch_state = 'fetched' and new.fetch_state = 'fetched' then
...
create trigger attachments_preserve_fetched_acquisition_metadata
before update of
  fetch_state,
  mime_type,
  size_bytes,
...
on public.attachments
for each row
execute function public.preserve_fetched_attachment_acquisition_metadata();

comment on function public.preserve_fetched_attachment_acquisition_metadata()
is 'Private trigger helper that prevents later source-metadata reconciliation from overwriting validated fetched-byte metadata.';

revoke all on function
  public.preserve_fetched_attachment_acquisition_metadata()
from public, anon, authenticated, service_role;

create function public.claim_teams_attachment_acquisition(
...
create function public.complete_teams_attachment_acquisition(
...
begin
  ...
  then
    raise exception using
      errcode = '22023',
      message = 'Attachment acquisition completion metadata is invalid.';
  end if;

  if (
    p_mime_type like 'image/%'
    and p_size_bytes > 10485760
  ) or (
    p_mime_type like 'audio/%'
    and p_size_bytes > 26214400
  )
  then
    raise exception using
      errcode = '22023',
      message = 'Attachment acquisition completion size is invalid.';
  end if;

  return query
  update public.attachments as completed_attachment
  set fetch_state = 'fetched',
  ...
  where completed_attachment.id = p_attachment_id
    and completed_attachment.fetch_state = 'downloading'
    and completed_attachment.fetch_lease_id = p_lease_id
    and p_storage_path = format(
      'teams/%s/%s/%s',
      completed_attachment.teams_message_id,
      completed_attachment.id,
      p_sha256
    )
  returning completed_attachment.id, completed_attachment.fetch_state;

  if not found then
    raise exception using
      errcode = 'P0001',
      message = 'Attachment acquisition completion transition was rejected.';
  end if;
end;
$$;

create function public.record_teams_attachment_acquisition_outcome(
  p_attachment_id uuid,
  p_lease_id uuid,
  p_outcome text,
  p_error_code text
...
is 'Service-role-only SKIP LOCKED claim with a bounded, reclaimable acquisition lease.';
comment on function public.complete_teams_attachment_acquisition(uuid, uuid, text, text, bigint, text)
is 'Service-role-only transition that finalizes validated bytes stored at the deterministic private object path.';
comment on function public.record_teams_attachment_acquisition_outcome(uuid, uuid, text, text)
is 'Service-role-only transition for safe retryable, permanent, or unsupported acquisition outcomes.';

revoke all on function
  public.claim_teams_attachment_acquisition(integer, integer),
  public.complete_teams_attachment_acquisition(
    uuid,
    uuid,
    text,
    text,
  ...
  public.record_teams_attachment_acquisition_outcome(
    uuid,
    uuid,
    text,
    text
...
-- Phase 3C makes mime_type and size_bytes acquisition-owned once bytes are
-- validated. Keep the Phase 3B source projection idempotent after that point.
create or replace function public.ingest_teams_message_core(
  p_message jsonb,
    ...
    select
      ...
      exists (
        ...
        where existing_attachment.teams_message_id = v_message_id
          ...
          and (
            existing_attachment.fetch_state = 'fetched'
            or (
              existing_attachment.mime_type is not distinct from
  ...
  if v_should_enqueue then
    ...
    loop
      ...
      do update set
        ...
        mime_type = case
          when attachments.fetch_state = 'fetched'
            then attachments.mime_type
...
comment on function public.ingest_teams_message_core(jsonb, jsonb) is
  'Private ingestion implementation. For fetched attachments, validated MIME type and exact byte size are acquisition-owned and excluded from Graph projection reconciliation.';
