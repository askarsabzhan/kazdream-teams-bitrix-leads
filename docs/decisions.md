...
## Phase 4C evidence-grounded group candidate extraction

Structured extraction runs once per current conversation-group identity and produces a validated group candidate, not a canonical lead. `lead_groups.candidate_payload` stores the last successfully completed candidate JSON alongside its `candidate_source_fingerprint`; separate extraction identity/target fields describe the current or next attempt. A newer failed identity therefore cannot erase or silently replace the successful candidate, its eligibility, completion time, metrics, or revision. Historical successful payloads before that current candidate are not archived in this MVP; immutable Teams/attachment evidence and versioned group-target evidence remain preserved. Cross-group canonical deduplication is deferred to Phase 4D.

The ordered input package contains only grouped Teams body/reply text and successful current transcript/OCR text. Stable PII-free references use `msg:<ordinal>:text` and `att:<ordinal>:transcript|ocr`; ambiguous messages and unsuccessful attachment content are excluded. The OpenAI Responses API uses the official SDK, `gpt-4o-mini` by default, no SDK retries, a closed Structured Outputs object, and a versioned evidence-only prompt. Every non-default result must cite input references, and application validation rejects unknown references, unsupported enums, and contact values that are not conservatively present in their cited source. Original phone/email spellings are retained while exact comparison-only normalization removes phone formatting or lowercases email; suspicious domains and missing country codes are never corrected.

`field_evidence` is reused with the smallest pre-lead extension: exactly one of `lead_id` or `lead_group_id` is required. Group rows record extraction revision, synthetic reference, resolvable message/attachment IDs, evidence method, and accepted/conflicted status without copying source text. Unresolved name conflict keeps the candidate name null and stores each underlying reference as conflicted evidence. Distinct reliable phones/emails remain separate values and are not treated as person-name conflicts.

Partner/Customer is derived after model output in deterministic application code. Explicit, non-negated partner, integrator, system integrator, seller, distributor, dealer, or supported Russian equivalents select `Partner` and retain their evidence references. Explicit customer/client/end-user language produces an evidence-grounded `Customer`. A genuine positive Partner/Customer contradiction is persisted as conflicted evidence and resolves safely to the mandatory Customer fallback; negated Partner terms never promote the group. With neither classification, `Customer` carries the documented `system:customer-default` provenance. Industry alone never implies Partner. This expanded reason/provenance contract is `group-candidate-schema-v2`, so a future worker run creates a controlled new extraction opportunity instead of silently applying changed post-processing under the prior identity. Campaign values are inherited from one authoritative application-level test configuration (`Hannover Messe 2026`, Bitrix value `63`, source `EXHIBITION`) and are never model-inferred. The candidate payload is the Phase 4D handoff source for those values; unrelated `campaigns` rows are not joined, and `lead_groups.campaign_id` remains null until the later canonical-lead/campaign boundary. Region is only `Europe` when source evidence clearly supports a European location; all other/unmapped regions are null. Product interests are limited to the six confirmed semantic values and require source support. Priority remains null unless explicit urgency/deadline context supports an allowed value.

Eligibility is deterministic: a supported full name plus at least one supported phone is `eligible`; otherwise the group remains `not_eligible` with `MISSING_FULL_NAME`, `MISSING_PHONE`, or `CONFLICTED_FULL_NAME`. Email and company are not hard requirements, and an ineligible group is retained for later evidence.

The extraction fingerprint covers group ID, grouping revision and algorithm version, ordered evidence content hashes, provider, model, prompt version, and schema version. An unchanged successful identity is a no-op. A changed identity creates a controlled target revision, but the successful extraction revision and `candidate_source_fingerprint` advance only after fenced completion. The original remotely applied migration cleared the successful payload while a changed identity was merely processing; a separate forward-only correction adds a before-update preservation guard and leaves the applied migration immutable. The existing `processing_jobs` table has one unique `process_lead_group` job per group/grouping revision: a trigger covers future group revisions and the Phase 4C migration backfills current groups. Worker-facing claim/complete/failure/verification RPCs are service-role-only `SECURITY DEFINER` functions with empty search paths; claims use `SKIP LOCKED`, bounded leases, fencing, and at most five durable attempts. Direct `service_role` mutation of groups, field evidence, and jobs remains prohibited.

## Phase 4D deterministic canonical lead resolution

Only current Phase 4C candidates with `eligibility.state = eligible` enter canonical resolution. Exact comparison-only normalized phone and email are strong identities. Exact supported normalized full name plus company is a secondary identity only; no fuzzy similarity, time proximity, manager identity, suspicious-domain correction, or missing-country-code repair is used. A strong key that points to more than one lead, or a phone-to-A/email-to-B split, persists `identity_conflict` on the group and does not create or merge a lead.

The existing `leads` and nullable `lead_groups.lead_id` relationship are reused. `lead_identity_keys` adds the database uniqueness boundary for `(kind, normalized_value)`. Resolution and recomposition run through service-role-only `SECURITY DEFINER` RPCs with an empty search path and one transaction advisory lock, so concurrent workers cannot create duplicate canonical identities. Application roles have no direct mutation access to the identity table or canonicalization RPCs.

Every linked current group participates in deterministic recomposition. Reliable phones, emails, relationship indicators, product interests, and facts are stable unions; supported scalar disagreement becomes null/conflicted instead of selecting an arbitrary value. Explicit Partner evidence remains Partner even when another group has only the mandatory Customer default. The campaign is the single fixed Hannover Messe 2026 / `63` / `EXHIBITION` configuration. The latest non-null contributor by actual Teams `source_created_at` and stable message-ID tie-break becomes responsible; no fabricated or administrative fallback is used. A late group enriches the existing lead regardless of arrival order. Canonical revision increments only when canonical payload, responsible contributor, or configured campaign meaningfully changes; exact replay is a no-op.

The final Russian analytical summary is derived from the canonical candidate plus source evidence of all linked groups. It uses one versioned Structured Outputs call for each canonical source fingerprint/provider/model/prompt identity, with SDK retries disabled. Claim/complete/failure RPCs use leases, `SKIP LOCKED`, fencing, five bounded durable attempts, and safe error codes. A successful unchanged identity cannot be claimed again; a meaningful canonical revision creates a new summary identity. Model output is untrusted, schema-bounded, and rejected if it contains internal evidence references. No Bitrix call, Teams write, deployment, or CRM enum inference occurs in Phase 4D.

## Phase 5 durable idempotent Bitrix synchronization

The existing `crm_outbox`, `manager_mappings`, canonical `leads.bitrix_lead_id`, and CRM status fields are reused. One forward-only migration adds lease fencing, five-attempt enforcement, blocked state, durable Bitrix-lead and timeline-comment stages, synced revision metadata, and an immutable `bitrix_source_group_id`. A trigger enqueues one `(lead_id, canonical revision, sync)` operation only after that revision has a successful summary; an exact replay is a unique-key no-op. A changed canonical revision resets local CRM state and receives a new outbox operation only after its replacement summary succeeds.

Every CLI run performs read-only `crm.lead.fields`, `crm.lead.userfield.list`, `crm.status.list`, and `user.get` discovery before claiming work. Confirmed enum IDs and labels, `SOURCE_ID=EXHIBITION`, standard lead fields, Teams provenance fields, and directory access must all pass. A discrepancy or authorization failure stops before Bitrix mutation. The webhook remains a server-only environment value and is neither persisted nor included in errors or logs.

Manager mapping is exact and durable: the latest canonical Teams AAD user ID resolves through Graph mail/userPrincipalName, then exact case-insensitive Bitrix email. One unique Bitrix ID is persisted; zero matches block with `MANAGER_MAPPING_MISSING`, multiple IDs block with `MANAGER_MAPPING_AMBIGUOUS`, and no admin/display-name fallback exists.

Remote lead creation uses the canonical lead's first immutable linked group ID in `UF_CRM_TEAMS_GROUP_ID`. Without a local Bitrix ID, lookup always precedes add: zero matches creates, one recovers and updates, and multiple matches block. This closes the add-success/local-finalization crash window. Lead delivery is durably finalized before the separate source timeline comment, so a retry can continue from the stored stage without recreating the lead.

`COMMENTS` contains the current Russian analytical summary. A separate timeline comment contains source Teams text, verbatim transcript, and useful OCR, prefixed with the PII-free revision marker `[KD-SOURCE:<canonical-lead-uuid>:r<revision>]`. Local outbox completion prevents intentional replay. Timeline comment creation still has weaker crash-recovery guarantees than lead creation because no clean Bitrix external-idempotency field for comments has been verified; Phase 5 deliberately avoids a large remote timeline reconciliation subsystem.
